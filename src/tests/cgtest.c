...
void cg_test_rectangle(void)
{
	...
	if(rect_width / 2 < radius)
	{
		...
		{
			cg_move_to(cgctx, x0, (y0 + y1) / 2);
			cg_curve_to(cgctx, x0, y0, x0, y0, (x0 + x1) / 2, y0);
			cg_curve_to(cgctx, x1, y0, x1, y0, x1, (y0 + y1) / 2);
			cg_curve_to(cgctx, x1, y1, x1, y1, (x1 + x0) / 2, y1);
			cg_curve_to(cgctx, x0, y1, x0, y1, x0, (y0 + y1) / 2);
		}
		else
		{
			cg_move_to(cgctx, x0, y0 + radius);
			cg_curve_to(cgctx, x0, y0, x0, y0, (x0 + x1) / 2, y0);
			cg_curve_to(cgctx, x1, y0, x1, y0, x1, y0 + radius);
			cg_line_to(cgctx, x1, y1 - radius);
			cg_curve_to(cgctx, x1, y1, x1, y1, (x1 + x0) / 2, y1);
			cg_curve_to(cgctx, x0, y1, x0, y1, x0, y1 - radius);
		}
	}
	else
	{
		...
		{
			cg_move_to(cgctx, x0, (y0 + y1) / 2);
			cg_curve_to(cgctx, x0, y0, x0, y0, x0 + radius, y0);
			cg_line_to(cgctx, x1 - radius, y0);
			cg_curve_to(cgctx, x1, y0, x1, y0, x1, (y0 + y1) / 2);
			cg_curve_to(cgctx, x1, y1, x1, y1, x1 - radius, y1);
			cg_line_to(cgctx, x0 + radius, y1);
			cg_curve_to(cgctx, x0, y1, x0, y1, x0, (y0 + y1) / 2);
		}
		else
		{
	...
	cg_set_source_rgb(cgctx, 0.5, 0.5, 1);
	cg_fill_preserve(cgctx);
	cg_set_source_rgba(cgctx, 0.5, 0, 0, 0.5);
	cg_set_line_width(cgctx, 10.0);
	cg_stroke(cgctx);
}
...
void cg_test_arc(void)
{
	...
	cg_set_source_rgba(cgctx, 1, 1, 0.5, 1);
	cg_set_line_width(cgctx, 10.0);
	cg_arc(cgctx, xc, yc, radius, angle1, angle2);
	cg_stroke(cgctx);

	cg_set_source_rgba(cgctx, 1, 0.2, 0.2, 0.6);
	cg_set_line_width(cgctx, 6.0);

	cg_arc(cgctx, xc, yc, 10.0, 0, 2 * M_PI);
	cg_fill(cgctx);

	cg_arc(cgctx, xc, yc, radius, angle1, angle1);
	cg_line_to(cgctx, xc, yc);
	cg_arc(cgctx, xc, yc, radius, angle2, angle2);
	cg_line_to(cgctx, xc, yc);
	cg_stroke(cgctx);
}
...
*/
			...
			cg_set_source_rgba(cgctx, cr/255., cg/255., cb/255., ca/255.);
			if(txt.c)
			{
...
*/
			...
			cg_move_to(cgctx, r->x1, r->y1);
			cg_line_to(cgctx, r->x2, r->y2);
			cg_stroke(cgctx);
			return 0;
		}
...
*/		
			...
			cg_rectangle(cgctx, r->x, r->y, r->width, r->height);
			cg_stroke(cgctx);
			return 0;
		}
...
typedef struct {
	...
	INT16	type;
			...
			if(angle1!=angle2)
				cg_arc_negative(cgctx, r->x, r->y, r->rx, angle1*(M_PI/(-180*64)), angle2*(M_PI/(-180*64)));
			else
				cg_ellipse(cgctx, r->x, r->y, r->rx, r->ry);
			if(r->type == GR_ARC)
				cg_stroke(cgctx);
			else
				cg_fill(cgctx);
...
*/
			...
			if(r->type & MWOUTLINE)
			{
				...
				cg_line_to(cgctx, r->x+r->ax, r->y+r->ay);
			}
			else
				cg_move_to(cgctx, r->x+r->ax, r->y+r->ay);
			cg_line_to(cgctx, r->x+r->bx, r->x+r->by);
			if(r->type & MWOUTLINE)
				cg_close_path(cgctx);
			cg_stroke(cgctx);
...
*/
			...
			cg_new_path(cgctx);
			cg_move_to(cgctx, p->x, p->y); ++p;
			for(int i = 1; i < len; ++i, ++p)
				cg_line_to(cgctx, p->x, p->y);
			cg_close_path(cgctx);
			cg_fill(cgctx);
			return 0;
		}
